...
def Man_Map_Maker():
    ...
    try: #Inputting the measurments of the map, also making sure that it fits the proper dimensions.
        map_size_x = int(input("How wide do you want the map to be? (X)|"))
        map_size_y = int(input("How long do you want the map to be? (Y)|"))
...
##            Man_Map_Maker()
    ...
    try:
        game_map[int(player_y) - 1][int(player_x) - 1] = "[p]"
    except(IndexError):
...
def Save_Map():
    global game_map, map_size_x, map_size_y,player_x,player_y
    save_name = input("What will you call the level? |")
    map_save = open((f'Maps/{save_name}.LPM'), "w")
